...
class DataParallelPPOActor(BasePPOActor):
    ...
    def _forward_micro_batch(
        self, micro_batch: dict[str, torch.Tensor], temperature: float, calculate_entropy: bool = False, calculate_logits: bool = False,
        ...
        with torch.autocast(device_type=self.device_name, dtype=self.param_dtype):
            ...
            if calculate_logits:
                outputs["prompt_logits"] = prompt_logits
    ...
    def compute_log_prob(self, data: DataProto, calculate_entropy: bool = False, calculate_logits: bool = False) -> dict[str, torch.Tensor]:
        ...
        for micro_batch in micro_batches:
            ...
            if calculate_logits:
                prompt_logits_lst.append(outputs["prompt_logits"])
    ...
    def update_policy(self, data: DataProto):
        ...
        if self.config.use_sft_loss and "prompt_mask" in data.batch.keys():
        ...
        for _ in range(self.config.ppo_epochs):
            for batch_idx, mini_batch in enumerate(mini_batches):
                ...
                for micro_batch in micro_batches:
                    ...
                    prompt_logits = outputs["prompt_logits"] if calculate_logits else None
                    ...
                    if self.config.use_sft_loss and "prompt_mask" in select_keys:
                        response_length = model_inputs["responses"].size(-1)
                        # prompt_mask=model_inputs["prompt_mask"][:, :-1] if "prompt_mask" in select_keys else torch.zeros_like(model_inputs["input_ids"][:, :-response_length - 1])
                            ...
                            micro_batch_metrics.update(
                                {
                                    ...
                                    "expert/pg_clipfrac": sft_metrics["pg_clipfrac"],
                                    "expert/ppo_kl": sft_metrics["ppo_kl"],
                                    "expert/pg_clipfrac_lower": sft_metrics["pg_clipfrac_lower"],
                                }
                            )
