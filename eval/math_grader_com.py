...
def last_boxed_only_string(string):
    boxed_space_idx = string.rfind("\\boxed ")
    boxed_brace_idx = string.rfind("\\boxed{")
    fbox_idx = string.rfind("\\fbox")

    idx = max(boxed_space_idx, boxed_brace_idx, fbox_idx)
    if idx < 0:
        return None

    if idx == boxed_space_idx:
        return "\\boxed " + string[idx + len("\\boxed ") :].split("$")[0].strip()
...
def answer_tag_reward_fn(model_response, gt_answer, fast=False):
    ...
    # if "</think>\n<answer>" in model_response and "</answer>" in model_response:
