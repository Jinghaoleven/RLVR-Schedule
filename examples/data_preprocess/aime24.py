...
if __name__ == "__main__":
    ...
    if local_dataset_path is not None:
        ...
        #     local_dataset_path,
        # )
        dataset = datasets.load_dataset("parquet", data_files=local_dataset_path)
    else:
        dataset = datasets.load_dataset(
            data_source,
        )
    ...
    def make_map_fn(split):
        def process_fn(example, idx):
            question = example.pop("prompt")[0]["content"].split(" Let's think step by step and output the final answer within \\boxed{}.")[0]
    ...
    train_dataset.to_parquet(os.path.join(args.local_save_dir, "aime24-system.parquet"))
