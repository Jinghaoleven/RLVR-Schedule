...
def process_jsonl_file(file_name):
    ...
    results = [{"gt": None, "responses":[]} for i in range(num)]
    with open(file_name) as f:        
        for line in f:
            data = json.loads(line)
            id = int(data["example_id"])
            gt = data["answer"][0] if isinstance(data["answer"], list) else data["answer"]
            response = data["response"]
            results[id]["gt"] = str(gt)
...
parser.add_argument("--file_dir", type=str, help='Path to load the dataset jsonl files', default="")
...
file_names = os.listdir(args.file_dir)
for file_name in file_names:
    file_name = os.path.join(args.file_dir,file_name)
    print(f"[INFO]: Evaluating: {file_name}")
