...
experiment_name=grpo_entropyadv_schedule
MODEL_PATH=${MODEL_PATH:-/mnt/public/users/zhangjinghao/models/Qwen3-4B}
TRAIN_FILE=${TRAIN_FILE:-$WORKING_DIR/dataset/OpenReasoning/train.parquet}
VAL_FILE=${VAL_FILE:-$WORKING_DIR/dataset/aime25/test.parquet}
...
if [ "${RANK}" == "0" ]; then
    ...
    ray job submit --address="http://$MASTER_ADDR:8265" \
        ...
        data.schedule_strategy=entropy_adv_schedule \
        algorithm.entropy_adv_schedule_alpha=0.4 \
        algorithm.entropy_adv_schedule_kappa=2.0 \
